...
model = load_model('hand_gesture_model5epoch.h5')  # Load pre-trained weights
modelfkrvwxyz=load_model('hand_gesture_modelfkrvwxyz.h5')
modelptz=load_model('hand_gesture_modelptz.h5')
def predict(input_data):
    predictions = model.predict(input_data)
    predicted_class = np.argmax(predictions)
    predicted_class = chr(predicted_class+ord('A'))

    fkrvwxyz=['F','K','R','V','W','X','Y','Z']
    ptz=['P','T','Z']
    if(predicted_class in fkrvwxyz):
        predictions = modelfkrvwxyz.predict(input_data)
        predicted_class = np.argmax(predictions)
        predicted_class = fkrvwxyz[predicted_class]
        if(predicted_class in ptz):
            predictions = modelptz.predict(input_data)
            predicted_class = np.argmax(predictions)
            predicted_class = ptz[predicted_class]
    if(predicted_class in ptz):
            predictions = modelptz.predict(input_data)
            predicted_class = np.argmax(predictions)
            predicted_class = ptz[predicted_class]
...
# TRACKBAR
        ...
        cv2.putText(frame, "Letter: "+predicted_class,
                    (102, 50), cv2.FONT_HERSHEY_TRIPLEX, 1, (0, 0, 255), 2)
